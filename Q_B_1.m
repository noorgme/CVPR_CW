...
%% 3D PCA Scatter with Principal Axes
nexttile;
...
quiver3(0, 0, 0, coeff(1,1), coeff(2,1), coeff(3,1), 'c', 'LineWidth', 2);
quiver3(0, 0, 0, coeff(1,2), coeff(2,2), coeff(3,2), 'k', 'LineWidth', 2);
quiver3(0, 0, 0, coeff(1,3), coeff(2,3), coeff(3,3), 'm', 'LineWidth', 2);
...
%% 1D Number Line for All Principal Components
nexttile;
scatter(score(normal_idx,1), ones(size(score(normal_idx,1))) * 3, 10, 'r', 'filled'); hold on;
scatter(score(TPU_idx,1), ones(size(score(TPU_idx,1))) * 3, 10, 'g', 'filled');
scatter(score(rubber_idx,1), ones(size(score(rubber_idx,1))) * 3, 10, 'b', 'filled');

scatter(score(normal_idx,2), ones(size(score(normal_idx,2))) * 2, 10, 'r', 'filled');
scatter(score(TPU_idx,2), ones(size(score(TPU_idx,2))) * 2, 10, 'g', 'filled');
scatter(score(rubber_idx,2), ones(size(score(rubber_idx,2))) * 2, 10, 'b', 'filled');

scatter(score(normal_idx,3), ones(size(score(normal_idx,3))) * 1, 10, 'r', 'filled');
scatter(score(TPU_idx,3), ones(size(score(TPU_idx,3))) * 1, 10, 'g', 'filled');
scatter(score(rubber_idx,3), ones(size(score(rubber_idx,3))) * 1, 10, 'b', 'filled');
